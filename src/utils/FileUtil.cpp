...
Error::Errc FileUtil::atomicWriteFile(const std::string& filepath, const std::vector<unsigned char>& data) {
...
#ifdef _WIN32
    {
        ...
        if (!ofs.is_open() || !ofs.good()) {
            SS_LOG_ERROR("Failed to open temporary file for writing (Windows fallback): " << tempFilepath << " - " << strerror(errno));
            return Error::Errc::FileOpenFailed;
        }
        if (!data.empty()) {
            ofs.write(reinterpret_cast<const char*>(data.data()), data.size());
            if (!ofs.good()) { 
                SS_LOG_ERROR("Failed to write data to temporary file (Windows fallback): " << tempFilepath << " - " << strerror(errno));
                ofs.close();
        ...
        ofs.close();
        if (ofs.fail() && !ofs.eof()) { 
             SS_LOG_ERROR("Error during close after writing temporary file (Windows fallback): " << tempFilepath << " - " << strerror(errno));
             std::remove(tempFilepath.c_str());
             return Error::Errc::FileWriteFailed;
        }
    }
#else // POSIX
    fd = open(tempFilepath.c_str(), O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH); // Permissions 0644
    if (fd < 0) {
        SS_LOG_ERROR("Failed to open temporary file '" << tempFilepath << "' for writing: " << strerror(errno));
        return Error::Errc::FileOpenFailed;
    }
    ...
    if (fsync(fd) != 0) {
        SS_LOG_ERROR("Failed to fsync temporary file '" << tempFilepath << "': " << strerror(errno));
        close(fd);
        std::remove(tempFilepath.c_str());
        return Error::Errc::FileWriteFailed; 
    }
    ...
    if (close(fd) != 0) {
        SS_LOG_ERROR("Failed to close temporary file '" << tempFilepath << "' after fsync: " << strerror(errno));
        std::remove(tempFilepath.c_str());
        return Error::Errc::FileWriteFailed; 
    }
#endif
    ...
    if (dir_fd < 0) {
        SS_LOG_WARN("Failed to open directory '" << dirToSync << "' for fsync: " << strerror(errno) 
                    << ". Rename operation might not be fully persistent on power loss.");
    } else {
        if (fsync(dir_fd) != 0) {
            SS_LOG_WARN("Failed to fsync directory '" << dirToSync << "': " << strerror(errno)
                        << ". Rename operation might not be fully persistent on power loss.");
        }
        close(dir_fd);
...
Error::Errc FileUtil::createDirectories(const std::string& path) {
    ...
    // Normalize path separators to '/' for internal processing, especially if mixed ones could come from tests.
    ...
    while (pos < path_copy.length()) {
        ...
        if (next_slash == std::string::npos) { // Last segment
            ...
            } else if (currentPath.empty()){ // Should not happen with corrected logic if path_copy is not empty
                continue;
            }
        }
        
        if (currentPath == "." || currentPath == "..") continue; // Skip . and ..
...
Error::Errc FileUtil::listDirectory(const std::string& directoryPath, std::vector<std::string>& files) {
    ...
    if (hFind == INVALID_HANDLE_VALUE) {
        SS_LOG_ERROR("Failed to open directory (Windows): " << directoryPath << " - Error: " << GetLastError());
        return Error::Errc::FileOpenFailed;
    }
    do {
        ...
        if (name != "." && name != "..") {
            ...
            if (!fullEntryPath.empty() && fullEntryPath.back() != '\\' && fullEntryPath.back() != '/') {
                fullEntryPath += '\\';
            }
            fullEntryPath += name;
